...
# CONFIG
INPUT_FILE = "data/raw_audio/output_cutnorm.wav"
# INPUT_FILE = "data/ref/squeezie.wav"
FFMPEG_PATH = "ffmpeg"
ASSETS_DIR = Path("data/raw_audio")
...
print(times, data, framerate, n_channels, samp_width)
...
def create_segment_file(segment, index, input_file, framerate):
    """Create a temporary WAV file for a single segment or silence."""
    out_file = TMP_DIR / f"segment_{index}.wav"
    if out_file.exists():
        out_file.unlink()
...
def generate_preview(segments):
    """Generate a preview WAV file by concatenating all segments."""
    print(segments)
    preview_file = TMP_DIR / "preview_temp.wav"
    final_preview = TMP_DIR / "final_preview.wav"
    ...
    segment_files = [create_segment_file(seg, i, INPUT_FILE, framerate) for i, seg in enumerate(segments)]

    concat_file = TMP_DIR / "concat_list.txt"
    with open(concat_file, 'w') as f:
        for segf in segment_files:
            f.write(f"file '{segf.resolve()}'\n")
...
def handle_preview_save(preview_click, save_click, segments):
    ...
    if button_id == 'generate-preview':
        ...
        try:
            final_preview = generate_preview(segments)
            data_uri = file_to_data_uri(final_preview)
            return "Preview generated.", data_uri
        except Exception as e:
            return f"Error generating preview: {e}", no_update
        ...
        try:
            final_preview = generate_preview(segments)
            final_file = ASSETS_DIR / "final_output.wav"
            if final_file.exists():
                final_file.unlink()
            normalize_audio(final_preview, final_file, target_db=-20)

            data_uri = file_to_data_uri(final_preview)
            return f"Final output saved as {final_file.name}.", data_uri
        except Exception as e:
            return f"Error saving output: {e}", no_update
