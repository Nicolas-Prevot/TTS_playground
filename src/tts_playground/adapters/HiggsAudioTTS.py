...
# Third-party dependencies for Higgs Audio
import torch
import langid
import jieba
import tqdm
...
# Base class from your project
from tts_playground.base import BaseTTS

# Higgs Audio specific imports
...
from tts_playground.adapters.HiggsAudio_audio_processing.higgs_audio_tokenizer import load_higgs_audio_tokenizer
    ...
    def load_model(self):
        ...
        if "cuda" in self.device and ":" in self.device:
            try:
                ...
                logger.warning(f"Could not parse device ID from '{self.device}'. Using default.")
        
        logger.info(f"Loading Higgs Audio model on device: {self.device}. {device_id=}")
